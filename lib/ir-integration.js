...
export async function offbyteWithIR(frontendPath, outputPath, options = {}) {
  console.log('\n' + '='.repeat(70));
  console.log('ðŸš€ offbyte - IR-Based Backend Generation');
...
Auto-generated with **offbyte IR Architecture**
...
\`\`\`bash
docker build -t offbyte-app .
docker run -p 3000:3000 offbyte-app
\`\`\`
...
Generated by offbyte IR Architecture
...
export async function quickGenerate(frontendPath, options = {}) {
  ...
  const outputPath = `./offbyte-${projectName}`;
...
export async function runFromCLI(args) {
  ...
  const out = outputPath || `./offbyte-${path.basename(frontendPath)}`;
